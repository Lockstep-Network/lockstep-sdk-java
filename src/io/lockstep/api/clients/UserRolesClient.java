...
public class UserRolesClient
{
    ...
    {
        RestRequest r = new RestRequest("GET", "/api/v1/UserRoles/{id}");
        r.AddPath("{id}", id);
        r.AddQuery("include", include);
        return this.client.Request<UserRoleModel>(r);
    }
    ...
    {
        RestRequest r = new RestRequest("GET", "/api/v1/UserRoles/query");
        r.AddQuery("filter", filter);
        r.AddQuery("include", include);
        r.AddQuery("order", order);
        r.AddQuery("pageSize", pageSize);
        r.AddQuery("pageNumber", pageNumber);
        return this.client.Request<FetchResult<UserRoleModel>>(r);
    }
}
